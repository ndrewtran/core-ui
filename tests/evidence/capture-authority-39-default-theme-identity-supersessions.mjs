...
const outputRoot = 'tests/evidence/authority-39-default-theme-identity';
const outputDirectoryName = 'authority-39-default-theme-identity';
const predecessorIndexPath = 'tests/evidence/authority-39-reset/index.json';
const decisionPath = 'decisions/0005-default-theme-token-source-identity.json';
const acceptancePath = 'decisions/0005-default-theme-token-source-identity-acceptance.json';
...
if (acceptance.decisionId !== 'core-ui:decision:0005' || acceptance.issueNumber !== 39 || acceptance.owner !== 'ndrewtran') {
  throw new Error('EVIDENCE_SUPERSESSION_AUTHORIZATION_INVALID: wrong decision 0005 owner or issue');
}
...
const topology = decision.evidenceTopology.authorityApplicabilitySupersession;
const predecessorIndex = await readJson(predecessorIndexPath);
const expectedPredecessorPaths = topology.targetNames.map((name) => `tests/evidence/authority-39-reset/supersessions/${name}.json`);
...
const targets = [];
for (const outputName of topology.targetNames) {
  const predecessorPath = `tests/evidence/authority-39-reset/supersessions/${outputName}.json`;
...
for (const entry of evidenceEntries.filter((candidate) => candidate.isDirectory() && candidate.name !== outputDirectoryName)) {
...
for (const target of targets) {
  ...
  const supersession = {
    ...
    replacementPlan: topology.replacementPlan,
...
console.log(`[authority-39-default-theme-identity] ${checkOnly ? 'verified' : 'captured'} ${references.length} append-only supersessions at ${sourceRevision}`);
