...
export async function resolveReviewReadinessSourceIdentity(repositoryRoot) {
  ...
  const sourceAt = async (revision) => {
    const parents = (await git(repositoryRoot, ['show', '-s', '--format=%P', revision])).trim().split(' ').filter(Boolean);
    if (parents.length !== 1 || parents[0] !== REVIEW_READINESS_ACCEPTED_BASE) return null;
    const decisionPaths = (await git(repositoryRoot, [
      'ls-tree', '--name-only', revision, '--', REVIEW_READINESS_DECISION, REVIEW_READINESS_ACCEPTANCE,
    ])).trim().split('\n').filter(Boolean);
    const evidenceRoot = (await git(repositoryRoot, ['ls-tree', '--name-only', revision, '--', REVIEW_READINESS_ROOT])).trim();
    if (decisionPaths.length !== 2 || evidenceRoot !== '') return null;
    return revision;
  };
  const evidenceSource = async (revision) => {
    const parents = (await git(repositoryRoot, ['show', '-s', '--format=%P', revision])).trim().split(' ').filter(Boolean);
    if (parents.length !== 1 || !(await sourceAt(parents[0]))) return null;
    const changes = (await git(repositoryRoot, ['diff', '--name-status', parents[0], revision])).trim().split('\n').filter(Boolean)
      .map((line) => line.split('\t'));
    const changedPaths = changes.map(([, path]) => path).sort((left, right) => left.localeCompare(right));
    const expectedPaths = [
      `${REVIEW_READINESS_ROOT}/index.json`,
      ...REVIEW_READINESS_TARGETS.map((target) => `${REVIEW_READINESS_ROOT}/supersessions/${target}.json`),
    ].sort((left, right) => left.localeCompare(right));
    if (changes.some(([status]) => status !== 'A') || canonicalJson(changedPaths) !== canonicalJson(expectedPaths)) return null;
    return parents[0];
  };
  let sourceRevision = await sourceAt(head);
  if (!sourceRevision) sourceRevision = await evidenceSource(head);
  if (!sourceRevision) {
    const parents = (await git(repositoryRoot, ['show', '-s', '--format=%P', head])).trim().split(' ').filter(Boolean);
    if (parents.length === 2 && parents.filter((parent) => parent === REVIEW_READINESS_ACCEPTED_BASE).length === 1) {
      const evidenceRevision = parents.find((parent) => parent !== REVIEW_READINESS_ACCEPTED_BASE);
      sourceRevision = await evidenceSource(evidenceRevision);
      if (sourceRevision) {
        const [headTree, evidenceTree] = await Promise.all([
          git(repositoryRoot, ['rev-parse', `${head}^{tree}`]),
          git(repositoryRoot, ['rev-parse', `${evidenceRevision}^{tree}`]),
        ]);
        if (headTree.trim() !== evidenceTree.trim()) sourceRevision = null;
      }
    }
  }
  if (!sourceRevision) fail('expected direct source, exact evidence child, or protected two-parent merge topology');
  return { sourceRevision, sourceTree: (await git(repositoryRoot, ['rev-parse', `${sourceRevision}^{tree}`])).trim() };
}
