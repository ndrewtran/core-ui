...
test('E-G0.4 CLI requires exact bindings and filters project-wide discovery', () => {
  ...
  assert.equal(response.meta.authority, 'installed-local');
  assert.deepEqual(response.meta.resolution.targetPackages, { '@core-ui/catalog': '0.3.0' });
...
test('E-G0.4 pnpm adapter normalizes renderer packages into the single resolver', async () => {
  ...
  try {
    ...
    const descriptor = {
      ...
      releaseProvenance: `core-ui-release:0.3.0:${bundle.sourceRevision}`,
      bindings: {
        [binding]: {
    ...
    const identity = {
      ...
      releaseManifest: {
        ...
        sourceRevision: bundle.sourceRevision,
        catalog: {
          id: `@core-ui/catalog@0.3.0:${bundle.catalogDigest}`,
          version: '0.3.0',
