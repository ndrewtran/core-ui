...
const producerBase = '65855ac805fe0f74798099d0d1ef47e860ba24a6';
...
test('validator re-derives source tree, lockfile, before-images, and patch identities', async () => {
  ...
  try {
    ...
    const resultForged = { ...structuredClone(original), result: { envelopeDigest: original.intentId, head: original.source.lockfile, tree: original.source.lockfile, diff: original.proposal.patch, changedPaths: [], operationJournal: original.source.lockfile, deterministicResults: [], reviewedInputs: [] } };
...
test('completed result binds the stable preview to the actual head, tree, diff, journal, and inputs', async () => {
  ...
  try {
    ...
    const { intentId: ignoredIntentId, ...intentPreimage } = produced.envelope;
    void ignoredIntentId;
    const previewSubject = { algorithm: 'sha256', digest: produced.envelope.intentId, byteLength: Buffer.byteLength(canonicalJson(intentPreimage)) };
    ...
    const reviewed = taskIdentity('completed-reviewed-input.json', { ...binding, profile: 'core-ui-review-input-v1', subject: previewSubject });
    const result = {
    ...
    for (const [field, child] of [
      ...
      ['reviewedInputs', { ...binding, profile: 'core-ui-review-input-v1', subject: { algorithm: 'sha256', digest: digestBytes(Buffer.from('forged-subject')), byteLength: previewSubject.byteLength } }],
    ]) {
      const forged = structuredClone({ ...produced.envelope, result });
      const descriptor = taskIdentity(`invalid-${field}.json`, child);
      if (field === 'deterministicResults' || field === 'reviewedInputs') forged.result[field] = [descriptor];
