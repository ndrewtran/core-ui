...
export const CHANGE_INTENT_BINDINGS = Object.freeze({
  authority: Object.freeze({ commit: '65855ac805fe0f74798099d0d1ef47e860ba24a6', tree: 'a84b77f804b79fa4a9fe8f630d98e63986e92ca7' }),
...
const AUTHORITY_PATHS = Object.freeze({ decision: 'decisions/0010-amendment-06-r1-change-intent-owner.md', acceptance: 'decisions/0010-amendment-06-r1-change-intent-owner-acceptance.md', architecture: 'strategy/monorepo-architecture.md', roadmap: 'strategy/milestone-roadmap.md', productScope: 'strategy/product-scope.md' });
...
function operationInput(input) { object(input, 'producer input'); const operation = input.operation ?? input; object(operation, 'operation'); const kind = operation.kind ?? operation.operationKind; if (!OPERATION_KINDS.has(kind)) fail('CORE_CHANGE_INTENT_OPERATION_INVALID', 'operation kind is not admitted'); const effectClass = { 'r1-lock': 'explanation-only', 'routine-git-operation': 'explanation-only', 'retained-evidence-acceptance': 'evidence-retention-write', 'project-migration': 'project-write', 'component-implementation': 'renderer-source-write' }[kind]; if (operation.effectClass !== undefined && operation.effectClass !== effectClass) fail('CORE_CHANGE_INTENT_EFFECT_INVALID', 'caller-selected effect is not accepted'); if (Object.hasOwn(operation, 'version') || Object.hasOwn(operation, 'versionEffect') || Object.hasOwn(operation, 'proof') || Object.hasOwn(operation, 'staleProof') || Object.hasOwn(operation, 'owner')) fail('CORE_CHANGE_INTENT_DERIVED_FIELD_INVALID', 'caller-selected version, proof, owner, or effect facts are not accepted'); if (kind === 'retained-evidence-acceptance' && operation.evidence === undefined) fail('CORE_CHANGE_INTENT_EVIDENCE_INVALID', 'retained evidence requires an exact evidence identity'); return { operation, kind, effectClass }; }
...
const COMPLETED_CHILD_PROFILES = Object.freeze({
  ...
  reviewed: 'core-ui-review-input-v1',
});
function validateCompletedChildRecord(record, expected, envelope, kind, childType, label) {
  const profiles = {
    ...
    reviewed: { profile: COMPLETED_CHILD_PROFILES.reviewed, keys: ['profile', 'envelopeDigest', 'head', 'tree', 'subject'] },
  };
  ...
  if (childType === 'reviewed') {
    const subject = record.subject;
    const expectedSubject = { algorithm: 'sha256', digest: expected.envelopeDigest, byteLength: Buffer.byteLength(canonicalJson(envelopePreimage(envelope))) };
    if (!isObject(subject) || Object.keys(subject).some((key) => !['algorithm', 'digest', 'byteLength'].includes(key)) || canonicalJson(subject) !== canonicalJson(expectedSubject)) fail('CORE_CHANGE_INTENT_RESULT_INVALID', `${label} does not bind the exact preview subject`, { subject, expectedSubject });
  }
  return record;
}
...
function completedResult(root, value, envelope, kind, expectedBase, expectedCommit) {
  ...
  exactObjectKeys(value, new Set(['envelopeDigest', 'head', 'tree', 'diff', 'changedPaths', 'operationJournal', 'deterministicResults', 'reviewedInputs']), 'envelope.result');
  ...
  const reviewedInputs = completedResultIdentities(root, value.reviewedInputs, 'envelope.result.reviewedInputs', envelope, expectedRecordBinding, kind, 'reviewed');
  return { envelopeDigest: value.envelopeDigest, head: expectedHead, tree: expectedTree, diff: expectedDiff, changedPaths: changed, operationJournal: journal, deterministicResults, reviewedInputs };
}
...
function schemaCheck(schema, value, at, rootSchema, issues) { if (schema.$ref) { const target = schema.$ref.startsWith('#/$defs/') ? rootSchema.$defs[schema.$ref.slice('#/$defs/'.length)] : undefined; if (!target) issues.push(`${at} unresolved schema reference`); else schemaCheck(target, value, at, rootSchema, issues); return; } if (schema.const !== undefined && canonicalJson(value) !== canonicalJson(schema.const)) issues.push(`${at} must equal ${JSON.stringify(schema.const)}`); if (schema.enum && !schema.enum.some((item) => canonicalJson(item) === canonicalJson(value))) issues.push(`${at} must use an admitted value`); if (schema.type && ![].concat(schema.type).some((itemType) => schemaType(value, itemType))) { issues.push(`${at} has wrong type`); return; } if (typeof value === 'string') { if (schema.minLength !== undefined && value.length < schema.minLength) issues.push(`${at} is too short`); if (schema.pattern && !new RegExp(schema.pattern, 'u').test(value)) issues.push(`${at} has invalid syntax`); } if (Array.isArray(value)) { if (schema.minItems !== undefined && value.length < schema.minItems) issues.push(`${at} has too few items`); if (schema.uniqueItems && new Set(value.map((item) => canonicalJson(item))).size !== value.length) issues.push(`${at} contains duplicates`); if (schema.items) value.forEach((item, index) => schemaCheck(schema.items, item, `${at}/${index}`, rootSchema, issues)); } if (isObject(value)) { for (const required of schema.required ?? []) if (!Object.hasOwn(value, required)) issues.push(`${at}/${required} is required`); const declared = new Set(Object.keys(schema.properties ?? {})); for (const key of Object.keys(value)) { if (schema.additionalProperties === false && !declared.has(key)) issues.push(`${at}/${key} is unknown`); if (declared.has(key)) schemaCheck(schema.properties[key], value[key], `${at}/${key}`, rootSchema, issues); } } }
export function validateChangeIntentEnvelope(value, { repositoryRoot, afterImages, expectedBase, expectedHead } = {}) {
  ...
  const { operation, kind, effectClass } = operationInput({ operation: value.operation });
  ...
  const expectedReview = { roles: reviewRoles(root, kind), independent: true, manual: kind === 'retained-evidence-acceptance' ? ['proof-owner review'] : ['profile-selected independent review'] };
...
async function produceChangeIntentEnvelope({ repositoryRoot, ...input } = {}) {
  const root = resolve(repositoryRoot ?? MODULE_REPOSITORY_ROOT);
  const { operation, kind, effectClass } = operationInput(input);
